...
// Headless test prototypu 9. Spouštět:
...
//   3. Big bang z 1 buňky funguje (svět expanduje)
...
function runScenario(name, programText) {
  ...
  for (let t = 1; t <= TICKS; t++) {
    ...
    if (t % checkpointEvery === 0) {
      ...
      const bbStr = bb ? `(${bb.xMin}..${bb.xMax}, ${bb.yMin}..${bb.yMax})` : "(empty)";
      const cStr = c ? `(${c.x.toFixed(2)}, ${c.y.toFixed(2)})` : "(empty)";
