...
const toroid = new ToroidWorld({ N: 64, seed: SEED, diffusionCoeff: 0.15, cpuK: 1, moveThreshold: 2.0 });
...
function dumpBoth(coord, t) {
  const [x, y] = coord;
  console.log(`--- (${x},${y}) tick ${t} ---`);
  dumpCell("  sparse", sparse.getCell(x, y));
  dumpCell("  toroid", toroid.getCell(x, y));
}

const coords = [[0, 0], [1, 0], [-1, 0], [0, 1], [0, -1], [2, 0], [-2, 0], [0, 2], [0, -2], [3, 0], [2, 1], [2, -1]];
