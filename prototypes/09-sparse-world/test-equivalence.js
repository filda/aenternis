...
// Equivalence test: spustí stejný program v sparse světě i v toroidu se
...
const N_TOROID = parseInt(process.env.N || "64", 10);
...
function compareTick(t) {
  ...
  // Výčet všech aktivních buněk v toroidu
  for (const tCell of toroid.cells) {
    const sCell = sparse.getCell(tCell.x, tCell.y);
    if (tCell.energy === 0) {
      if (sCell && sCell.energy > 0) {
        return { diverged: true, reason: `toroid (${tCell.x},${tCell.y}) má E=0, sparse má E=${sCell.energy}`, t };
      }
      continue;
    }
    if (!sCell) {
      return { diverged: true, reason: `toroid (${tCell.x},${tCell.y}) má E=${tCell.energy}, sparse buňka neexistuje`, t };
    }
    if (sCell.energy !== tCell.energy) {
      return { diverged: true, reason: `(${tCell.x},${tCell.y}) energie: toroid=${tCell.energy}, sparse=${sCell.energy}`, t };
    }
    if (sCell.memory.length !== tCell.memory.length) {
      return { diverged: true, reason: `(${tCell.x},${tCell.y}) memSize: toroid=${tCell.memory.length}, sparse=${sCell.memory.length}`, t };
    }
    for (let i = 0; i < tCell.memory.length; i++) {
      if (sCell.memory[i] !== tCell.memory[i]) {
        return { diverged: true, reason: `(${tCell.x},${tCell.y}) mem[${i}]: toroid=0x${tCell.memory[i].toString(16)}, sparse=0x${sCell.memory[i].toString(16)}`, t };
      }
    }
    if (sCell.pc !== tCell.pc) {
      return { diverged: true, reason: `(${tCell.x},${tCell.y}) pc: toroid=${tCell.pc}, sparse=${sCell.pc}`, t };
    }
    for (let d = 0; d < 4; d++) {
      if (sCell.pointers[d] !== tCell.pointers[d]) {
        return { diverged: true, reason: `(${tCell.x},${tCell.y}) ptr[${d}]: toroid=${tCell.pointers[d]}, sparse=${sCell.pointers[d]}`, t };
      }
    }
  }
...
console.log(`Equivalence test: program=${PROG}, E_total=${E_TOTAL}, ticks=${TICKS}, N_toroid=${N_TOROID}, seed=${SEED}`);
