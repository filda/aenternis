...
// Toroidní 2D svět pro comparison harness proti sparse světu.
// Port prototypu 6 s jediným rozdílem: per-cell RNG seedovaný stejnou
// hash funkcí jako sparse svět (cellSeed). Tím obě fyziky sdílí stejný
...
const LAYOUT_ORDER_FROM_END = [3, 2, 1, 0];
...
class ToroidWorld {
  constructor(opts = {}) {
    ...
    // Souřadnice toroidu: středovaný kolem (0,0). Cell (x, y) má index
    // ((y + N/2) * N + (x + N/2)). Bbox toroidu: [-N/2, N/2 - 1] na obou osách.
    this.half = Math.floor(this.N / 2);
    for (let y = 0; y < this.N; y++) {
      for (let x = 0; x < this.N; x++) {
        const cx = x - this.half;
        const cy = y - this.half;
        this.cells[y * this.N + x] = makeCell(cx, cy, this.worldSeed);
      }
    }
  }
  ...
  bigBang(eTotal, programSlots = []) {
    ...
    for (let i = 0; i < this.cells.length; i++) {
      ...
      cell.pointers = [0, 0, 0, 0];
      cell.rates = [0, 0, 0, 0];
      cell.activeOutflow = [0, 0, 0, 0];
      cell.pointerOverridden = [false, false, false, false];
      cell.pc = 0;
      cell.lastInst = null;
      cell.instCount = 0;
      cell.tickBudget = 0;
  ...
  activeBbox() {
    ...
    return count === 0 ? null : { xMin, xMax, yMin, yMax, count };
  }
  ...
  step() {
    ...
    for (let i = 0; i < total; i++) {
      ...
      for (let d = 0; d < DIRS; d++) {
        const [dx, dy] = DIR_OFFSET[d];
        const nIdx = this.idx(cell.x + dx, cell.y + dy);
