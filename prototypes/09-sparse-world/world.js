...
//
// Datová struktura: Map<bigint coord, Cell>, kde coord je zabalené (x, y) do
// 64-bit bigint. Souřadnice jsou 32-bit signed (záporné jsou potřeba — svět
// expanduje všemi směry od (0, 0)).
//
// Fyzika přejatá z prototypu 6 (2D toroid, 20 opcodů). Jediný rozdíl je
// topologie: místo modulárního lookupu sousedů hash mapa, neexistující sousedi
// jsou prostě absent (E_neighbor = 0), nově se alokují při prvním inflow.
...
// ----- Coord packing -----
//
// Souřadnice jsou 32-bit signed integer per osa. Pro Map klíč zabalíme do
// 64-bit BigInt: high 32 bity = x, low 32 bity = y. Záporné se masknou na
// uint32 reprezentaci, aby BigInt operace byly konzistentní.

const MASK32 = 0xFFFFFFFFn;

function packCoord(x, y) {
  return ((BigInt(x | 0) & MASK32) << 32n) | (BigInt(y | 0) & MASK32);
}

function unpackCoord(key) {
  // Reverze packCoord. Sign extend přes Int32Array trick.
  const xRaw = Number((key >> 32n) & MASK32) | 0;
  const yRaw = Number(key & MASK32) | 0;
  return [xRaw, yRaw];
}
...
function cellSeed(worldSeed, x, y) {
...
function cellTickSeed(worldSeed, x, y, tick) {
  let h = cellSeed(worldSeed, x, y);
...
function makeCell(x, y, worldSeed) {
  const seed = cellSeed(worldSeed, x, y);
  return {
    x, y,
    energy: 0,
...
class SparseWorld {
  ...
  getCell(x, y) {
    return this.cells.get(packCoord(x, y));
  }

  hasCell(x, y) {
    return this.cells.has(packCoord(x, y));
  }

  // Alokace nové buňky s prázdnou pamětí (E = 0). Volá se z inflow fáze
  // při prvním zápisu do dosud neexistující pozice.
  allocateCell(x, y) {
    const cell = makeCell(x, y, this.worldSeed);
    this.cells.set(packCoord(x, y), cell);
    return cell;
  }
  ...
  getOrCreate(x, y) {
    ...
    if (!cell) {
      cell = makeCell(x, y, this.worldSeed);
  ...
  // Big bang: jedna buňka v (0, 0) drží celé E_total. Volitelný program
  ...
  bigBang(eTotal, programSlots = []) {
    ...
    const cell = this.allocateCell(0, 0);
    cell.energy = eTotal;
  ...
  centroid() {
    ...
    return { x: sumX / sumE, y: sumY / sumE };
  }

  // ----- CPU fáze (přejato z prototypu 6) -----
  ...
  recomputeAllLayouts() {
    ...
    for (const cell of this.cells.values()) {
      ...
      const rng = makeRng(cellTickSeed(this.worldSeed, cell.x, cell.y, this.tick));
  ...
  step() {
    ...
    const combinedRatesByCell = new Map(); // bigint -> [4]
    for (const cell of this.cells.values()) {
      const memSize = cell.memory.length;
      ...
      combinedRatesByCell.set(packCoord(cell.x, cell.y), rates);
    ...
    for (const out of outflows) {
      ...
      const [dx, dy] = DIR_OFFSET[out.dir];
      const tx = src.x + dx;
      const ty = src.y + dy;
      const key = packCoord(tx, ty);
    ...
    for (const cell of this.cells.values()) {
      preStepEnergy.set(packCoord(cell.x, cell.y), cell.energy);
    }

    const allTargets = new Set();
    // Existing cells s outflow
    for (const cell of this.cells.values()) allTargets.add(packCoord(cell.x, cell.y));
    ...
    for (const targetKey of allTargets) {
      const [tx, ty] = unpackCoord(targetKey);
      let cell = this.cells.get(targetKey);
      const isNew = !cell;
      if (isNew) {
        cell = this.allocateCell(tx, ty);
      }
      ...
      for (const inf of bucket) {
        ...
        const nKey = packCoord(neighbor.x, neighbor.y);
    ...
    for (const cell of this.cells.values()) {
      cell.activeOutflow[0] = 0; cell.activeOutflow[1] = 0;
      cell.activeOutflow[2] = 0; cell.activeOutflow[3] = 0;
      cell.pointerOverridden[0] = false; cell.pointerOverridden[1] = false;
      cell.pointerOverridden[2] = false; cell.pointerOverridden[3] = false;
    }
...
// ----- Asembler (přejato z prototypu 6) -----
