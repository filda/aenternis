...
function reset() {
  state.N = clamp(parseInt(dom.size.value, 10), 8, 48);
...
function initScenario() {
  ...
  if (scenario === "point") {
    setCellEnergy(c, c, c, maxE, randomBytes(maxE));
    ...
    for (let z = 0; z < state.N; z++)
      for (let y = 0; y < state.N; y++)
        for (let x = 0; x < state.N; x++) {
          const e = Math.floor(Math.random() * (maxE / 2));
          if (e > 0) setCellEnergy(x, y, z, e, randomBytes(e));
        }
...
function formatMemory(cell) {
  ...
  // Per řádek: 8 slotů (každý 8 hex znaků), aby řádek byl ~72 znaků
  const PER_LINE = 8;
  const lines = [];
  let line = [];
  let lineStart = 0;
  for (let i = 0; i < memSize; i++) {
    ...
    const isPc = i === pcAddr;
    let dirLabels = "";
    cell.pointers.forEach((p, d) => { if (p === i) dirLabels += DIR_NAMES[d][0]; });
    let cls = "memory-slot";
    if (isPc && isPtr) cls += " memory-pc-and-pointer";
    else if (isPc) cls += " memory-pc";
    else if (isPtr) cls += " memory-pointer-marker";
    let title = "";
    if (isPc && isPtr) title = `title="PC + ${dirLabels} ptr"`;
    else if (isPc) title = `title="PC"`;
    else if (isPtr) title = `title="${dirLabels} ptr"`;
    line.push(`<span class="${cls}" ${title}>${hexSlot(mem[i])}</span>`);
  }
