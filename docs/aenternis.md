...
**Done:** diffusion + VM tick, dominance / intrusion (collision as soft mixing), WASM + Three.js viewer, inspector, program injection, the dense decode-fold instruction set, gravity / pressure / density-coupled mutation (`mechanics.md`, "Gravity and pressure"), the Rust core of procedural macro-genesis (`genesis-plan.md`), and the viewer's war-paint / lineage color modes (`appearance` + `origin_tag`, `src/color.ts`). **Backlog:** sensor opcodes (`sinflow`, `sself`, `srate` — only the inflow-tracking data exists so far), lineage tracker + manual tag (the color modes they build on are done), persistence, and the TypeScript / UI layer of macro-genesis.
